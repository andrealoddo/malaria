...
%Augmentation
trainImgs = augmentedImageDatastore([100 100], trainImgs,'ColorPreprocessing','gray2rgb');
testImgs = augmentedImageDatastore([100 100], testImgs,'ColorPreprocessing','gray2rgb');

% Training
[malarianet,info] = trainNetwork(trainImgs,layers,options);
trainAcc(i) = info.TrainingAccuracy(end);
trainLoss(i) = info.TrainingLoss(end);

% Classificazione delle immagini
testpreds = classify(malarianet,testImgs);
